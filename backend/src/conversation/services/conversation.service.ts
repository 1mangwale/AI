...
export class ConversationService {
  ...
  /**
   * Process message (LEGACY)
   * 
   * @deprecated This method is part of the legacy conversation flow system.
   * New implementations should use MessageGatewayService which routes through
   * ContextRouterService → FlowEngineService/AgentOrchestratorService.
   * 
   * Migration path:
   * - Phase 1: MessageGateway architecture (COMPLETE)
   * - Phase 2: Deprecate legacy handlers (IN PROGRESS)
   * - Phase 3: Remove this service entirely
   * 
   * @see MessageGatewayService
   * @see ContextRouterService
   */
  async processMessage(phoneNumber: string, message: any): Promise<void> {
    try {
      ...
      switch (session.currentStep) {
        ...
        // ⚠️ Deprecated auth steps archived in _archived/legacy-flow-system/conversation-auth-steps.ts
        // Any legacy auth step now routes through the default handler (AgentOrchestrator/flow engine)
        ...
        case 'parcel_delivery_ai':
          // AI-Powered Parcel Delivery - Route through unified Agent Orchestrator
          // The AgentOrchestratorService uses FlowEngineService with parcel-delivery.flow.ts
          // This replaces the isolated ParcelAgentService that made external calls
          const parcelResult = await this.agentOrchestratorService.processMessage(
            phoneNumber,
            messageText,
            ModuleType.PARCEL, // module
            null,
            undefined, // no testSession
          );
          
          if (parcelResult && parcelResult.response) {
            const platform = session?.data?.platform || 'whatsapp';
            const platformEnum = platform === 'web' ? Platform.WEB : 
                                platform === 'telegram' ? Platform.TELEGRAM : 
                                Platform.WHATSAPP;
            await this.messagingService.sendTextMessage(platformEnum, phoneNumber, parcelResult.response);
          }
          break;
        ...
        // ⚠️ Deprecated parcel/local-delivery steps archived in
        // _archived/legacy-flow-system/conversation-parcel-logic.ts
        // These now fall through to default to be handled by the flow engine / Agent Orchestrator
   ...
   * Routes through AgentOrchestrator -> FlowEngine (auth_v1 flow).
   * All auth goes through PHP backend via auth.executor.ts.
   */
  private async handleLoginMethod(phoneNumber: string, messageText: string): Promise<void> {
    ...
    // Route all auth through AgentOrchestrator -> FlowEngine (auth_v1)
    this.logger.log(`🔐 Routing auth to FlowEngine for ${phoneNumber}`);
    try {
      if (this.agentOrchestratorService) {
        const result = await this.agentOrchestratorService.processMessage(
          phoneNumber,
          'login',
          ModuleType.FOOD,
        ...
        if (result && result.response) {
          const loginSession = await this.sessionService.getSession(phoneNumber);
          const platform = loginSession?.data?.platform || 'whatsapp';
          const platformEnum = platform === 'web' ? Platform.WEB : 
                              platform === 'telegram' ? Platform.TELEGRAM : 
                              Platform.WHATSAPP;
          await this.messagingService.sendTextMessage(platformEnum, phoneNumber, result.response);
        }
      } else {
        this.logger.error(`❌ AgentOrchestrator not available for auth`);
        await this.messagingService.sendTextMessage(Platform.WHATSAPP, 
          phoneNumber,
          'Please try again later. Authentication service is temporarily unavailable.'
        );
      }
    } catch (error) {
      this.logger.error(`❌ FlowEngine auth failed: ${error.message}`);
      await this.messagingService.sendTextMessage(Platform.WHATSAPP, 
        phoneNumber,
  ...
  /**
   * @deprecated REMOVED - All auth now goes through FlowEngine (auth_v1).
   * Kept as stub for any sessions stuck in legacy steps.
   */
  private async handlePhoneNumberInput(phoneNumber: string, messageText: string): Promise<void> {
    this.logger.warn(`⚠️ DEPRECATED handlePhoneNumberInput called for ${phoneNumber} - routing to flow engine`);
    await this.handleLoginMethod(phoneNumber, messageText);
    return;
    // --- Original deprecated code below (unreachable) ---
   ...
   * 
   * @deprecated MIGRATION: This method should route to FlowEngine auth_v1 flow
  ...
  private async handleOtpVerification(phoneNumber: string, messageText: string): Promise<void> {
    // 🔧 FIX: Route to FlowEngine instead of duplicate methods
    this.logger.log(`🔐 Routing OTP verification to FlowEngine for ${phoneNumber}`);
    ...
    try {
      ...
      if (hasActiveAuthFlow && this.agentOrchestratorService) {
        // Continue existing auth flow
        this.logger.log(`🔄 Continuing existing auth flow`);
        const result = await this.agentOrchestratorService.processMessage(
          phoneNumber,
          messageText,
          ModuleType.FOOD,
      ...
      // No active flow - check if we need phone or OTP
      const loginMethod = await this.sessionService.getData(phoneNumber, 'login_method');
      const otpPhone = await this.sessionService.getData(phoneNumber, 'otp_phone');
      
      if (loginMethod === 'otp' && !otpPhone) {
        // User needs to enter phone number first - start auth flow
        if (this.agentOrchestratorService) {
          const result = await this.agentOrchestratorService.processMessage(
            phoneNumber,
        ...
        // User is entering OTP code - continue auth flow
        if (this.agentOrchestratorService) {
          const result = await this.agentOrchestratorService.processMessage(
            phoneNumber,
            messageText, // OTP code
            ModuleType.FOOD,
          );
          if (result && result.response) {
            const platform = session?.data?.platform || 'whatsapp';
      ...
      this.logger.error(`❌ FlowEngine auth routing failed: ${error.message}`);
   ...
   * 
   * @deprecated Legacy parcel handler. Use parcel-delivery.flow.ts via FlowEngineService instead.
   * @see backend/src/flow-engine/flows/parcel-delivery.flow.ts
   */
  ...
  private async handlePaymentMethodSelection(phoneNumber: string, messageText: string): Promise<void> {
    ...
    try {
      ...
      const orderPayload = {
        ...
        total_charge: deliveryCharge + (parseFloat(process.env.PLATFORM_FEE) || 5), // delivery_charge + platform_fee
        dm_tips: 0,
